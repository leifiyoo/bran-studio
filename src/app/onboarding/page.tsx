    ...
    <main className="grid min-h-screen place-items-center p-6">
      <section className="panel w-full max-w-4xl rounded-2xl p-8">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <label className="block text-sm font-medium">Theme<select className="mt-2 h-9 rounded-md border bg-background px-3 text-sm" value={theme} onChange={(e) => setTheme(e.target.value)}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label>
          <label className="block text-sm font-medium">Starting template<select className="mt-2 h-9 rounded-md border bg-background px-3 text-sm" value={template} onChange={(e) => setTemplate(e.target.value as typeof template)}><option value="blank">Blank project</option><option value="saas">SaaS dashboard starter</option><option value="mobile">Mobile app starter</option><option value="landing">Landing page starter</option></select></label>
        </div>
        ...
        <div className="mt-8 flex justify-end"><Button onClick={finish}>Finish and create project</Button></div>
      </section>
    </main>
  )
}
