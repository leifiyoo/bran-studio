...
export function fillPaint(node: SceneNode) {
  ...
  if (fill.type === 'linear-gradient' || fill.type === 'radial-gradient') return `url(#fill-${node.id})`
  return 'transparent'
}
...
export function FillDefs({ node }: { node: SceneNode }) {
  ...
  if (fill.type === 'radial-gradient') return <radialGradient id={`fill-${node.id}`} cx={`${fill.center.x}%`} cy={`${fill.center.y}%`} r={`${fill.radius}%`}>{fill.stops.map((stop, index) => <stop key={index} offset={`${stop.position}%`} stopColor={stop.color} stopOpacity={stop.alpha * fill.alpha}/>)}</radialGradient>
  return null
}

export function SceneNodeRenderer({ node, onPointerDown, onDoubleClick }: { node: SceneNode; selected: boolean; onPointerDown: (event: React.PointerEvent, node: SceneNode) => void; onDoubleClick: (node: SceneNode) => void }) {
  ...
  const cssFilter = [
    ...node.effects.map((effect) => effect.type === 'drop-shadow' ? `drop-shadow(${effect.x}px ${effect.y}px ${effect.blur}px color-mix(in srgb, ${effect.color} ${Math.round(effect.alpha * 100)}%, transparent))` : effect.type === 'layer-blur' ? `blur(${effect.blur}px)` : effect.type === 'filter' ? `${effect.filter}(${effect.value}${effect.filter === 'hue-rotate' ? 'deg' : '%'})` : ''),
  ...
  const stroke = node.strokes[0]?.color ?? 'transparent'
  const strokeWidth = node.strokes[0]?.width ?? 0
  const strokeOpacity = node.strokes[0]?.alpha ?? 1
  ...
  if (node.type === 'text') {
    return <g {...common}><foreignObject width={node.width} height={node.height}><div style={{fontFamily: `${node.fontFamily}, Geist, Inter, Arial, sans-serif`, fontSize: node.fontSize, fontWeight: node.fontWeight, letterSpacing: node.letterSpacing, lineHeight: `${node.lineHeight}px`, color: node.color, whiteSpace: 'pre-wrap', overflow: 'hidden', textAlign: node.textAlignHorizontal as never}}>{node.text}</div></foreignObject></g>
  }
  if (node.type === 'image') {
